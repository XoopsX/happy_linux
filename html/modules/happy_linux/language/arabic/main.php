<?php
// $Id: main.php,v 1.1 2010/11/07 14:59:14 ohwada Exp $

// 2006-09-01 K.OHWADA
// _HAPPY_LINUX_SEARCH_CANDICATE

// 2006-07-10 K.OHWADA
// this is new file

//=========================================================
// Happy Linux Framework Module
// 2006-07-10 K.OHWADA
//=========================================================

// form
define('_HAPPY_LINUX_TITLE','ÇáÚäæÇä');
define('_HAPPY_LINUX_SAVE', 'ÍÝÙ');
define('_HAPPY_LINUX_VISIT','Visit');

// page_frame
define('_HAPPY_LINUX_THERE_ARE','íæÌÏ <b>%s</b> ÓÌá');
define('_HAPPY_LINUX_NO_RECORD','áÇ íæÌÏ ÓÌáÇÊ');
define('_HAPPY_LINUX_ID_ASC', 'ÊÑÊíÈ ÊÕÇÚÏí');
define('_HAPPY_LINUX_ID_DESC','ÊÑÊíÈ ÊäÇÒáí');

// manage
define('_HAPPY_LINUX_ADD_RECORD','ÃÖÝ ÓÌá');
define('_HAPPY_LINUX_MOD_RECORD','ÊÚÏíá ÓÌá');
define('_HAPPY_LINUX_DEL_RECORD','ÍÐÝ ÓÌá');
define('_HAPPY_LINUX_ADD_RECORD_SUCCEEED','ÃÖíÝ ÈäÌÇÍ !');
define('_HAPPY_LINUX_MOD_RECORD_SUCCEEED','ÚÏá ÈäÌÇÍ !');
define('_HAPPY_LINUX_DEL_RECORD_SUCCEEED','ÍÐÝ ÈäÌÇÍ !');
define('_HAPPY_LINUX_ADD_RECORD_FAILD','ÝÔá Ýí ÇáÅÖÇÝÉ');
define('_HAPPY_LINUX_MOD_RECORD_FAILD','ÝÔá Ýí ÇáÊÚÏíá');
define('_HAPPY_LINUX_DEL_RECORD_FAILD','ÝÔá Ýí ÇáÍÐÝ');
define('_HAPPY_LINUX_ERR_FILL',   'ÎØÃ : ÏÎæá %s');
define('_HAPPY_LINUX_ERR_ILLEGAL','ÎØÃ : ÛíÑ ÞÇäæäí %s');

// global
define('_HAPPY_LINUX_HOME','ÇáÕÝÍÉ ÇáÑÆíÓíÉ');
define('_HAPPY_LINUX_MAIN','ÇáÑÆíÓíÉ');
define('_HAPPY_LINUX_ADDED','ÃÖíÝ');
define('_HAPPY_LINUX_DELETED','ÍÐÝ');
define('_HAPPY_LINUX_UPDATE', 'ÍÏË');
define('_HAPPY_LINUX_UPDATED','ÍÏË');
define('_HAPPY_LINUX_MODIFY',  'ÊÚÏíá');
define('_HAPPY_LINUX_MODIFIED','ÚÏá');
define('_HAPPY_LINUX_SAVED','ÍÝÙ');
define('_HAPPY_LINUX_CLEAR',  'ãÓÍ');
define('_HAPPY_LINUX_CLEARED','ãÓÍ');
define('_HAPPY_LINUX_EXECUTE', 'äÝÐ');
define('_HAPPY_LINUX_EXECUTED','äÝÐ');
define('_HAPPY_LINUX_CREATE', 'íäÔÃ');
define('_HAPPY_LINUX_CREATED','ÃäÔÇÁ');
define('_HAPPY_LINUX_SEARCH','ÈÍË');
define('_HAPPY_LINUX_SHOW','ÚÑÖ');
define('_HAPPY_LINUX_KEYWORD','ßáãÉ');
define('_HAPPY_LINUX_NUM','ÑÞã');
define('_HAPPY_LINUX_NO_ACTION','ÛíÑ äÔØ');

// 2006-09-01
define('_HAPPY_LINUX_SEARCH_CANDICATE','Candicate for search');
define('_HAPPY_LINUX_ZENKAKU','Zenkaku');
define('_HAPPY_LINUX_HANKAKU','Hanhaku');

?>